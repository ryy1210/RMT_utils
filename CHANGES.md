...
- `get_bulk_ks`を追加。固定した分散と閾値を使い、閾値以下の固有値を再正規化してKSを計算。
- `bulk_diagnostics.py`にthin SVDを再利用するDE計算、保存済み入力照合、途中保存・再開を追加。
- 保存済みKSの履歴、再計算した全体KS、バルクKS、バルク比率、分散・閾値を同じCSVへ記録。
- 分散1の診断には専用のMP+TW閾値を使用。元BEMA閾値と混同しない。
- 新規Notebookの既定値はCPU人工行列テスト。本番モデル取得とW&B送信は明示設定が必要。
- 本番用の保存先はDrive、W&B新規run。既存run・既存実験Notebookは変更しない。

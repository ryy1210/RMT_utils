...
def ppl_eval2(model, tokenizer, datasets=['wikitext2','ptb','c4'],
    ...
    with torch.inference_mode():  # no_gradよりも軽量（書き換え不要なら推奨）
        for dataset in datasets:
            test_loader = get_test_data(dataset, tokenizer,
                                        seq_len=model_seq_len,
                                        batch_size=batch_size)

            total_loss_sum = 0.0   # 全トークンの損失合計
            total_tokens   = 0     # トークン数
            ...
            for step, batch in pbar:
                ...
                if not torch.isfinite(logits).all():
                    pbar.set_postfix_str("non-finite logits -> skip")
                    continue
                ...
                total_loss_sum += loss.sum().item()
                total_tokens   += loss.numel()

                if step % log_every == 0:
                    cur_ppl = math.exp(total_loss_sum / max(1, total_tokens))
                    pbar.set_postfix({"ppl": f"{cur_ppl:.2f}"})
            ppl = math.exp(total_loss_sum / max(1, total_tokens))
            ppls[dataset] = ppl
            print(f"[{dataset}] PPL = {ppl:.3f}")
...
def ppl_eval3(model, tokenizer, datasets=['wikitext2','ptb','c4'],
              model_seq_len=2048, batch_size=32, device="cuda",
              log_every=20):
    # model.to(device)  # ← 削除！
    model.eval()

    # モデルが分割配置されていても安全に入力先デバイスを取得
    try:
        embed_device = model.model.embed_tokens.weight.device
    except Exception:
        embed_device = next(model.parameters()).device  # フォールバック
    ...
    with torch.inference_mode():
        for dataset in datasets:
            ...
            for step, batch in pbar:
                # dictバッチ/テンソルバッチの両対応で、入力だけ embed_device に移動
                if isinstance(batch, dict):
                    batch = {k: v.to(embed_device, non_blocking=True) for k, v in batch.items()}
                    input_ids = batch.get("input_ids", None)
                    attention_mask = batch.get("attention_mask", None)
                else:
                    batch = batch.to(embed_device, non_blocking=True)
                    input_ids = batch
                    attention_mask = None
